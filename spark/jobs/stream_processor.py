...
from fraud_scorer import build_alert_reason, compute_risk_score
...
def write_to_oltp(df, epoch_id: int):
    ...
    try:
        ...
        ).write.format("jdbc").option("url", POSTGRES_URL).option(
            "dbtable", "stream_staging"
        ).option("driver", "org.postgresql.Driver").option(
            "user", POSTGRES_USER
        ).option("password", POSTGRES_PASSWORD).mode("append").save()
        ...
        if fraud_df.count() > 0:
            fraud_df.select(
                ...
                "dbtable", "fraud_alerts"
            ).option("driver", "org.postgresql.Driver").option(
                "user", POSTGRES_USER
            ).option("password", POSTGRES_PASSWORD).mode("append").save()

        log.info(
            "Batch %d: wrote %d rows (%d fraud alerts)",
            epoch_id,
            len(rows),
            fraud_df.count(),
        )
...
def main():
    spark = (
        SparkSession.builder.appName("FraudLens-StreamProcessor")
    ...
    query = (
        enriched.writeStream.trigger(processingTime="5 seconds")
